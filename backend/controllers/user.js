...
export const signin = async (req, res) => {
  ...
  try {
    ...
    if (!existingUser) return res .status(404) .json({ message: `User with email ${email} does't exist` });
...
export const signup = async (req, res) => {
    ...
    try {
        const existingUser = await User.findOne({email})
        if(existingUser) return res.status(400).json({message : "User is allready exist"})
